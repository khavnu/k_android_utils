...
public class IndicatorStayLayout extends LinearLayout {
    ...
    private void layoutIndicator(View child, int index) {
        if (child instanceof IndicatorSeekBar) {
            ...
            View indicatorView = seekBar.getIndicator().getInsideContentView();
            if(indicatorView != null) {
                indicatorView.setOnTouchListener(new OnTouchListener() {
                    @Override
                    public boolean onTouch(View view, MotionEvent event) {
                        float limitRight = getWidth() - view.getWidth();
                        float limitLeft = 0;
                        float seekbarMaxProgress = seekBar.getMax();
                        switch (event.getActionMasked()) {
                            ...
                            case MotionEvent.ACTION_MOVE:
                                ...
                                float progress = 0;
                                if (limitRight > 0) {
                                    progress = ((newX * 100) / limitRight);
                                }
                                float seekbarProgress = ((progress/100) * seekbarMaxProgress);

                                seekBar.setProgressWithTickSmoothly(seekbarProgress);
