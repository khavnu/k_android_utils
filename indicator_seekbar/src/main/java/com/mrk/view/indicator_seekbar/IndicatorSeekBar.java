package com.mrk.view.indicator_seekbar;
...
import com.mrk.indicator_seekbar.R;
...
public class IndicatorSeekBar extends View {
    ...
    private Bitmap mUnselectTickMarksBitmapHighlight;//the drawable bitmap for tick highlight
    private Bitmap mSelectTickMarksBitmapHighlight;//the drawable bitmap for tick highlight
    ...
    private Bitmap mThumbBitmapHighlight;//the drawable bitmap for thumb highlight
    private int mThumbColor;
    private int mThumbSize;
    private Drawable mThumbDrawable;
    private Bitmap mPressedThumbBitmap;//the bitmap for pressing status
    private Bitmap mPressedThumbBitmapHighlight;//the bitmap for pressing status highlight
    ...
    private void drawTickMarks(Canvas canvas) {
        ...
        int centerPos = mTickMarksX.length/2;
        int endPos = mTickMarksX.length;
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            ...
            if (mTickMarksDrawable != null) {
                ...
                if(i == 0 || i == centerPos || i == endPos -1) {
                    //highlight Bitmap
                    float topPos = mProgressTrack.top - mUnselectTickMarksBitmapHighlight.getHeight() / 2.0f;
                    float leftPos = mTickMarksX[i] - mUnselectTickMarksBitmapHighlight.getWidth() / 2.0f;
                    if (i <= thumbPosFloat) {
                        canvas.drawBitmap(mSelectTickMarksBitmapHighlight, leftPos, topPos, mStockPaint);
                    } else {
                        canvas.drawBitmap(mUnselectTickMarksBitmapHighlight, leftPos, topPos, mStockPaint);
                    }
                } else {
                    float topPos = mProgressTrack.top - mUnselectTickMarksBitmap.getHeight() / 2.0f;
                    float leftPos = mTickMarksX[i] - mUnselectTickMarksBitmap.getWidth() / 2.0f;
                    if (i <= thumbPosFloat) {
                        canvas.drawBitmap(mSelectTickMarksBitmap, leftPos , topPos, mStockPaint);
                    } else {
                        canvas.drawBitmap(mUnselectTickMarksBitmap, leftPos, topPos, mStockPaint);
                    }
                }
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            if (i == 0) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] + mTickTextsWidth[index] / 2.0f, (int) (mTickTextY*1.5f), mTextPaint);
            } else if (i == mTickTextsArr.length - 1) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] - mTickTextsWidth[index] / 2.0f, (int) (mTickTextY*1.5f), mTextPaint);
            } else {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i], (int) (mTickTextY*1.5f), mTextPaint);
            }
        }
    }
    ...
    private void drawThumb(Canvas canvas) {
        ...
        if (mThumbDrawable != null) {//check user has set thumb drawable or not.ThumbDrawable first, thumb color for later.
            ...
            if (mIsTouching) {
                ...
                float leftPos = thumbCenterX - mThumbBitmap.getWidth() / 2.0f;
                boolean isStart = leftPos < 70;
                boolean isEnd = leftPos > (canvas.getWidth() - 70);

                boolean isCenter = ((leftPos > (canvas.getWidth()/2 - 30)) && (leftPos < (canvas.getWidth()/2 + 30)));  //Center

                boolean isHighlightThumb = isStart || isCenter || isEnd;

                if(isHighlightThumb) {
                    canvas.drawBitmap(mThumbBitmapHighlight, leftPos, mProgressTrack.top - mThumbBitmapHighlight.getHeight() / 2.0f, mStockPaint);
                } else {
                    canvas.drawBitmap(mThumbBitmap, leftPos, mProgressTrack.top - mThumbBitmap.getHeight() / 2.0f, mStockPaint);
                }
            }
        } else {
            if (mIsTouching) {
    ...
    private Bitmap getDrawBitmap(Drawable drawable, float scaleRatio, boolean isThumb) {
        if (drawable == null) {
            return null;
        }
        int width;
        ...
        int newHeight = (int) scaleRatio * height;

        Bitmap bitmap = Bitmap.createBitmap(width, newHeight, Bitmap.Config.ARGB_8888);
    ...
    private void initThumbBitmap() {
        ...
        if (mThumbDrawable instanceof StateListDrawable) {
            try {
                    ...
                    for (int i = 0; i < stateCount; i++) {
                        ...
                        if (stateSet.length > 0) {
                            if (stateSet[0] == android.R.attr.state_pressed) {
                                ...
                                mPressedThumbBitmap = getDrawBitmap(stateDrawable, 1, true);
                                mPressedThumbBitmapHighlight = getDrawBitmap(stateDrawable, 3, true);
                            } else {
                            ...
                            Drawable stateDrawable = (Drawable) getStateDrawable.invoke(listDrawable, i);
                            mThumbBitmap = getDrawBitmap(stateDrawable, 1, true);
                            mThumbBitmapHighlight = getDrawBitmap(stateDrawable, 3, true);
                        }
                    }
                } else {
                    //please check your selector drawable's format, please see above to correct.
                ...
                mThumbBitmap = getDrawBitmap(mThumbDrawable, 1, true);
                mPressedThumbBitmap = mThumbBitmap;

                mThumbBitmapHighlight = getDrawBitmap(mThumbDrawable, 3, true);
                mPressedThumbBitmapHighlight = mThumbBitmapHighlight;
            }
        } else {
            mThumbBitmap = getDrawBitmap(mThumbDrawable, 1, true);
            mPressedThumbBitmap = mThumbBitmap;
    ...
    private void initTickMarksBitmap() {
        if (mTickMarksDrawable instanceof StateListDrawable) {
            ...
            try {
                    ...
                    for (int i = 0; i < stateCount; i++) {
                        ...
                        if (stateSet.length > 0) {
                            if (stateSet[0] == android.R.attr.state_selected) {
                                Drawable stateDrawable = (Drawable) getStateDrawable.invoke(listDrawable, i);
                                mSelectTickMarksBitmap = getDrawBitmap(stateDrawable, 1, false);
                                mSelectTickMarksBitmapHighlight = getDrawBitmap(stateDrawable, 3, false);
                            } else {
                                //please check your selector drawable's format, please see above to correct.
                            ...
                            Drawable stateDrawable = (Drawable) getStateDrawable.invoke(listDrawable, i);
                            mUnselectTickMarksBitmap = getDrawBitmap(stateDrawable, 1, false);
                            mUnselectTickMarksBitmapHighlight = getDrawBitmap(stateDrawable, 3, false);
                        }
                    }
                } else {
                    //please check your selector drawable's format, please see above to correct.
                ...
                mUnselectTickMarksBitmap = getDrawBitmap(mTickMarksDrawable, 1, false);
                mSelectTickMarksBitmap = mUnselectTickMarksBitmap;

                mUnselectTickMarksBitmapHighlight = getDrawBitmap(mTickMarksDrawable, 3, false);
                mSelectTickMarksBitmapHighlight = mUnselectTickMarksBitmapHighlight;
            }
        } else {
            mUnselectTickMarksBitmap = getDrawBitmap(mTickMarksDrawable, 1, false);
            mSelectTickMarksBitmap = mUnselectTickMarksBitmap;
    ...
    float minIndicatorValue = 0.0f;
    public void setMinIndicatorValue(float _minIndicatorValue) {
        this.minIndicatorValue = _minIndicatorValue;
    }
    ...
    String getIndicatorTextString() {
        if (mIndicatorTextFormat != null && mIndicatorTextFormat.contains(FORMAT_TICK_TEXT)) {
            ...
            if (mProgress < minIndicatorValue) {
                return mIndicatorTextFormat.replace(FORMAT_PROGRESS, getProgressString(minIndicatorValue));
            } else {
    ...
    public synchronized void setProgressWithTickSmoothly(float progress) {
        mProgress = progress;
        autoAdjustThumb();
    }
    ...
    public static Bitmap scaleHeightBitmap(Bitmap  input, float heightScale) {
        Bitmap result = Bitmap.createScaledBitmap(input, (int) (input.getWidth()), (int) (input.getHeight()*heightScale), false);
        return result;
    }
}